...
def pointer_decoder(decoder_inputs, initial_state, attention_states, cell,
                    num_heads=1, dtype=tf.float32,
                    scope=None, feed_prev=False):
  """
  ...
  Args:
    ...
    output_size: size of the output vectors; if None, we use cell.input_size.
    ...
    loop_function: if not None, this function will be applied to i-th output
      ...
      Signature -- loop_function(prev, i) = next
        * prev is a 2D Tensor of shape [batch_size x cell.input_size],
        * i is an integer, the step number (when advanced control is needed),
        * next is a 2D Tensor of shape [batch_size x cell.output_size].
  ...
  with tf.variable_scope(scope or "ptrnet_decoder"):
    ...
    for i in xrange(len(decoder_inputs)):
      ...
      x = rnn_cell._linear([inp] + attns, cell.output_size, True, scope="inp_to_hidden")
